...
class ParcelPackage(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if "weight_uom_id" not in fields_list or defaults.get("weight_uom_id"):
            return defaults
        shipment_id = defaults.get("shipment_id") or self.env.context.get(
            "default_shipment_id"
        )
        if not shipment_id:
            return defaults
        shipment = self.env["parcel.shipment"].browse(shipment_id).exists()
        shipment.check_access("read")
        if shipment:
            defaults["weight_uom_id"] = (
    ...
    def create(self, vals_list):
        ...
        default_shipment_id = self.env.context.get("default_shipment_id")
        prepared = []
        for incoming in vals_list:
            values = dict(incoming)
            if "shipment_id" not in values and default_shipment_id:
                values["shipment_id"] = default_shipment_id
        ...
        shipments = self.env["parcel.shipment"].browse(shipment_ids)
        shipments.check_access("write")
        shipments = shipments.exists()
        if len(shipments) != len(shipment_ids) or any(
            not values.get("shipment_id") for values in prepared
        ):
            raise ValidationError(_("A valid shipment is required."))
        shipments.mapped("company_id")._lock_package_limits()
        ...
        for values in prepared:
            ...
            shipment = shipments_by_id[values["shipment_id"]]
            if not values.get("weight_uom_id"):
    ...
    def write(self, vals):
        ...
        if {"weight", "weight_uom_id"}.intersection(vals):
            self.mapped("company_id")._lock_package_limits()
        shipments = shipments._lock_shipments()
